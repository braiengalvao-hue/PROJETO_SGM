...
<body class="bg-light">
    <main class="container mt-4 d-flex flex-column align-items-center">
        <div class="d-flex justify-content-between align-items-center bg-primary text-white p-3 w-50 shadow rounded-top">
            <h1 class="h4 mb-0">Abrir Chamado</h1>
            <button class="btn btn-light btn-sm" onclick="window.location.href='solicitante_dashboard.php'">
                <i class="bi bi-arrow-left"></i> Voltar
            </button>
        </div>

        <form id="formChamado" class="p-4 border bg-white w-50 shadow rounded-bottom">
            <div class="mb-3">
            ...
            <div class="mb-3">
                ...
                <textarea id="descricao" class="form-control" rows="4" placeholder="Descreva o problema..." required></textarea>
            </div>

            <div class="mb-3">
                <label class="form-label fw-bold">Anexar Foto (Opcional)</label>
                <input type="file" id="foto" class="form-control" accept="image/*">
            </div>

            <button type="submit" class="btn btn-primary w-100 fw-bold py-2">Registrar solicitação</button>
        </form>
    </main>
  ...
  // Carrega Blocos e Tipos ao iniciar
        async function iniciar() {
            // Blocos
            const resB = await fetch('controllers/localizacoes.php?acao=listar_blocos');
            ...
            // Tipos
            const resT = await fetch('controllers/localizacoes.php?acao=listar_tipos');
        ...
        async function carregarAmbientes(id_bloco) {
            ...
            const res = await fetch(`controllers/localizacoes.php?acao=listar_ambientes&id_bloco=${id_bloco}`);
...
document.getElementById('formChamado').addEventListener('submit', async (e) => {
    ...
    const response = await fetch('controllers/salvar_chamado.php', {
        method: 'POST',
         body: formData
    });
